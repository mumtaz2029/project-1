pipeline {
    ...
    environment {
        ...
        DOCKERHUB_USERNAME = 'mumtaz2029'  // Set this directly to your Docker Hub username
        DOCKER_IMAGE = "${DOCKERHUB_USERNAME}/employee-portal:latest"
    }
        ...
        stage('Sonar Analysis') {
            steps {
                withSonarQubeEnv('sonar-server') {
                    sh "$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=employeeapp -Dsonar.projectKey=mumkey -Dsonar.java.binaries=target"
                }
            }
        }
